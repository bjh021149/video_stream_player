...
class ObjectBoxService {
  ...
  /// 批量从播放列表移除视频
  void removeMultipleVideosFromPlaylist(String playlistId, List<String> videoIds) {
    final playlist = getPlaylist(playlistId);
    
    if (playlist != null) {
      playlist.videos.removeWhere((v) => videoIds.contains(v.videoId));
      playlist.updatedAt = DateTime.now();
      playlistBox.put(playlist);
      _notifyListeners();
    }
  ...
  /// 批量添加视频到播放列表
  void addMultipleVideosToPlaylist(String playlistId, List<VideoEntity> videos) {
    final playlist = getPlaylist(playlistId);
  ...
  /// 复制播放列表
  PlaylistEntity duplicatePlaylist(String playlistId, String newName) {
    final original = getPlaylist(playlistId);
    if (original == null) throw Exception('播放列表不存在');
    ...
    // 复制视频列表
    newPlaylist.videos.addAll(original.videos);
    
    final id = playlistBox.put(newPlaylist);
    _notifyListeners();
    return playlistBox.get(id)!;
  }
